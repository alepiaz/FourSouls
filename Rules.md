...
Abilities
Abilities consist of non-gray colored text, called effect text, that is written in a card’s text box. There are different types of abilities that function in certain ways. Abilities only function while an object is in play, unless otherwise specified. If an ability is activated or triggered and put on the stack, and the object that created it subsequently leaves play before that ability resolves, the ability is not removed from the stack and will still take effect when it resolves. Loot abilities are an exception – they are performed when a loot resolves.
...
HP Counters
HP counters are a special type of counter. If a HP counter is placed on an item or character, it gives that item or character the ability “You have +1HP.” If a HP counter is placed on a monster, it gives that monster the ability “This has +1HP.” An object can have more than one HP counter at a time. If an object later loses a HP counter, it also loses the ability that HP counter was giving it.

ATK Counters
ATK counters are a special type of counter. If a ATK counter is placed on an item or character, it gives that item or character the ability “You have +1ATK.” If a ATK counter is placed on a monster, it gives that monster the ability “This has +1ATK.” An object can have more than one ATK counter at a time. If an object later loses a ATK counter, it also loses the ability that ATK counter was giving it.
...
The Recharge Step. The active player recharges (turns upright) objects they control. Any objects controlled by the game (with the exception of shop items) also recharge.
Abilities that trigger at the start of the turn trigger, then priority passes between all players.
The Loot Step. The active player loots 1. At the end of this step, priority passes between all players, then the action phase begins.
The Action Phase
...
Abilities that trigger at the end of the turn trigger, then priority passes between all players.
If the active player has more loot cards in their hand than their max hand size (10 by default), they discard loot cards until they have as many cards left in their hand as their max hand size.
If playing with the room deck, and if a monster died during the turn, the active player may put a room into discard.
The turn ends and is passed to the next player. All objects with an HP stat heal to full HP, including any dead players, and abilities and effects that last till end of turn end.
...
Some monsters or other objects may create an effect that forces a player to declare an attack on a specific target. This target must then always be chosen as the target of the attack declaration, for as long as the effect still applies (e.g. the effect may only force you to attack that target once each turn, and therefore if a player first attacked the forced target, managed to cancel that attack, and had the ability to declare an additional attack, they could then declare that second attack on a different target). If more than one of these effects are active at once, such that a player would be forced to declare an attack on two different targets, the player declaring the attack may choose which of the forced targets to declare the attack against.
...
Active Items
Active items are items that have at least one ↷ ability. Active and paid items both have gold borders.

Paid Items
Paid items are items that have at least one $ ability. Active and paid items both have gold borders.

There are a few cards that have both ↷ and $ abilities. In these edge cases, as is the general rule above, you can simply pick one of the two categories yourself, or refer to that card’s page on our website (foursouls.com).
...
For the former, the template used is “↷: Destroy this. If you do,”.
For the latter, the template used is “$: Destroy this:”.
Soul Items
...
Pills/Runes	6
Tarot Cards + Miscellaneous Wild Cards	23
Trinkets	11
Lost Soul	1
Pennies
...
Nickels
Most Nickels have the loot ability ‘Gain 5¢’. Similar to pennies, there are some that may also have additional effects. Loot cards that are included in this category have ‘Nickel’ in their name. For the sake of deck ratios, loot cards with ‘Dime’ in their name are also included in this category.
...
Tarot Cards + Miscellaneous Wild Cards
...
Bosses
Bosses tend to be harder to kill than other monsters, but yield greater rewards, as well as souls! Boss cards can be identified as they have both a stat block and a soul icon. For the sake of deck ratios, there are also a few cards that are considered ‘bosses’ despite not having a soul icon. These cards can be found on the website (foursouls.com).
