from foursouls.engine.events import AllPlayersPassed, EffectFizzled, PriorityPassed, StackItemResolved
...
def test_resolves_lifo_order_two_items():
    ...
    g.push_to_stack(controller_id=PlayerId("P1"), source="test", effect=AppendMarkerEffect("A"), label="A")
    g.push_to_stack(controller_id=PlayerId("P1"), source="test", effect=AppendMarkerEffect("B"), label="B")
...
def test_all_players_passed_emitted_when_stack_empty():
    g = _game_two_players_active_p1()
