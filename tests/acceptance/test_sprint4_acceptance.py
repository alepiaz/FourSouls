"""
Sprint 4 acceptance — combat: attack, roll, death, rewards, souls.
Includes R4.3 combat-roll loop tests (legality, hit/miss damage, events).
...
from foursouls.engine.events import CoinsGained, CombatRollResult, MonsterDied, PlayerDied, SoulGranted
...
# ---------------------------------------------------------------------------
# Shared helpers — integration path
# ---------------------------------------------------------------------------
...
def test_death_combat_cleared_on_player_death():
    ...
    g.step(AttackMonster(slot_index=0))
    g.step(RollCombat())    # miss → CombatMissDamageEffect on stack
    g.step(PassPriority())  # resolves → hp 1→0 → player dies
...
def test_sprint4_integration_full_death_flow():
    ...
    # Drive only the combat (attack + one roll + resolve); stop before EndTurn.
    combat_events: list = []
    combat_events.extend(g.step(AttackMonster(slot_index=0)).events)
    combat_events.extend(g.step(RollCombat()).events)   # miss → CombatMissDamageEffect on stack
    # Pass priority for both players to resolve the effect
    for _ in range(2):
        combat_events.extend(g.step(PassPriority()).events)
...
# ---------------------------------------------------------------------------
# R4.3 — Combat roll loop tests
# ---------------------------------------------------------------------------
# Strategy: evade=1 → guaranteed hit; evade=7 → guaranteed miss.

def _monster_stub(name: str, *, hp: int = 3, evade: int = 1) -> MonsterInPlay:
...
def _make_zones(monsters: list) -> GameZones:
...
def _combat_game(
    *,
    ...
    monsters = [_monster_stub("m-0", hp=monster_hp, evade=evade)]
    players = [
        PlayerState(player_id=PlayerId(f"P{i+1}"), max_hp=player_hp, hp=player_hp)
        for i in range(n_players)
    ]
...
def test_hit_reduces_monster_hp_by_1():
    g = _combat_game(monster_hp=3, evade=1)
...
def test_miss_reduces_player_hp_by_1():
    ...
    hp_before = g.state.get_player(PlayerId("P1")).hp
    g.step(RollCombat())
    g.step(PassPriority())   # resolve CombatMissDamageEffect
...
def test_two_misses_reduce_player_hp_by_2():
    g = _combat_game(player_hp=5, evade=7)
    g.step(RollCombat())
    g.step(PassPriority())   # resolve first miss
    g.step(RollCombat())
    g.step(PassPriority())   # resolve second miss
...
def test_player_hp_never_goes_below_zero():
    ...
    g.step(RollCombat())   # miss → CombatMissDamageEffect on stack
    g.step(PassPriority()) # resolves → hp 1 → 0, player dies, combat ends
    assert g.state.get_player(PlayerId("P1")).hp == 0
    assert g.combat is None
